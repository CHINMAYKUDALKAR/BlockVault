...
const hasJwt = (): boolean => {
  try {
    const stored = localStorage.getItem('blockvault_user');
    if (!stored) return false;
    const parsed = JSON.parse(stored);
    return Boolean(parsed?.jwt);
  } catch (error) {
    console.warn('Failed to read stored user for blockchain auth check', error);
    return false;
  }
};
...
async function safeRequest<T>(request: () => Promise<T>, fallback: T): Promise<T> {
  try {
    ...
    if (error?.response?.status === 401) {
      console.debug('[blockchainService] skipping unauthorized response');
      return fallback;
    }
    throw error;
  }
