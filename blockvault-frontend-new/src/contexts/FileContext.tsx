...
export const FileProvider: React.FC<FileProviderProps> = ({ children }) => {
  ...
  const refreshFiles = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.debug('Skipping files refresh - not authenticated');
        setFiles([]);
      }
    } catch (err) {
  ...
  const refreshSharedFiles = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.debug('Skipping shared files refresh - not authenticated');
        setSharedFiles([]);
      }
    } catch (err) {
  ...
  const refreshOutgoingShares = useCallback(async () => {
    try {
      ...
      if (response.ok) {
        ...
        console.debug('Skipping outgoing shares refresh - not authenticated');
        setOutgoingShares([]);
      }
    } catch (err) {
  ...
  // Poll for new shared files so recipients receive updates automatically
  useEffect(() => {
    const user = JSON.parse(localStorage.getItem('blockvault_user') || '{}');
    if (!user.jwt) {
      return;
